...
git checkout main
git checkout -b phase/02-storage-settings  # Backend Rust agent
...
| Backend Rust agent | `phase/02-storage-settings` | Define all Tauri command interfaces, service traits, error variants, model structs | Medium — Rust only, no DB deps |

**Why these two are safe in parallel:**
- Frontend agent only touches `src/**`
- Backend agent only touches `src-tauri/src/**`
- Zero file overlap
...
|---|---|---|
| Database/privacy agent | Backend contracts (Phase 2 commands/models) | Needs service interfaces defined before writing repositories |
...
1. **Create branches** `phase/01-ui-shell` and `phase/02-storage-settings` from `main`
2. **Push branches** to `origin`
3. **Launch Wave 1 agents** with their first tasks as defined in `docs/PARALLEL_EXECUTION_PLAN.md`
4. **Optionally create** `phase/qa-setup` for the QA agent
